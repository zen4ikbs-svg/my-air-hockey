...
RED, GREEN = (255, 50, 50), (0, 255, 120)
CYAN, DARK_BG = (0, 255, 255), (10, 12, 16)
WALL_COLOR = (30, 35, 45)
...
fonts = {
    ...
    "stats": pygame.font.Font(None, 28),
...
START_SPEED, MAX_SPEED = 1450, 3300 
...
def generate_skin_particles(pos, skin):
    ...
    elif skin == "Электро" and random.random() > 0.5:
        skin_effects.append({
            "pos": [pos[0] + random.uniform(-P_R, P_R), pos[1] + random.uniform(-P_R, P_R)],
            "life": random.randint(5, 15), "type": "electro"
        })
...
while running:
    ...
    if state == MENU:
        ...
        if state == GAME:
            if countdown <= 0:
                ...
                if ball_pos[0] < B_R + WALL_T or ball_pos[0] > W - B_R - WALL_T:
                    ball_vel[0] *= -1.06; ball_pos[0] = B_R + WALL_T + 1 if ball_pos[0] < W/2 else W - B_R - WALL_T - 1
            ...
            if ball_pos[1] < WALL_T or ball_pos[1] > H-WALL_T:
                if gx < ball_pos[0] < gx + G_W:
                    ...
                    ball_vel[1] *= -1.06; ball_pos[1] = WALL_T + 2 if ball_pos[1] < WALL_T else H - WALL_T - 2

        for eff in skin_effects[:]:
            if eff["type"] == "fire":
                pygame.draw.circle(screen, eff["color"], (int(eff["pos"][0]), int(eff["pos"][1])), int(eff["rad"]))
                eff["pos"][0] += eff["vel"][0]; eff["pos"][1] += eff["vel"][1]; eff["rad"] -= 0.5
        ...
        if current_skin == "КОРОЛЬ":
            ...
            crown_p = [(cx-35, cy-15), (cx-45, cy-55), (cx-15, cy-30), (cx, cy-65), (cx+15, cy-30), (cx+45, cy-55), (cx+35, cy-15)]
            pygame.draw.polygon(screen, GOLD, crown_p); pygame.draw.polygon(screen, WHITE, crown_p, 2) 
